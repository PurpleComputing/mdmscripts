...
latestver=$(curl -s ${releaseNotesUrl} | grep 'title="v' | cut -f4 -d'"' | sed -e 's/[a-zA-Z]//' )
...
if [[ $architecture == "arm64" ]]; then
	...
	downloadUrl="https://github.com/balena-io/etcher/releases/download/v${latestver}/balenaEtcher-${latestver}.dmg"
else
...
####################################################################################################
	...
	if [[ "${installMethod}" = "DMG" ]]; then
		...
		# Mount the DMG
       	...
       	ditto -rsrc "/Volumes/${appName} ${latestver}/${appName}.app" "/Applications/${appName}.app"
       	/bin/sleep 10
