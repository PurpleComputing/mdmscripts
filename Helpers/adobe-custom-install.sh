...
#
####################################################################################################
# adobe-custom-install.sh SCMv
# Last Updated by Michael Tanner, 19/04/2023
####################################################################################################
echo "*** BEGIN adobe-custom-install.sh ***"

label="customadobecc" # if no label is sent to the script, this will be used

echo Downloading Purple Logo
rm -rf /Library/Application\ Support/Purple/purple.icns
curl -o /Library/Application\ Support/Purple/purple.icns https://raw.githubusercontent.com/PurpleComputing/mdmscripts/main/PurpleLogos/purple.icns
export PATH=/usr/bin:/bin:/usr/sbin:/sbin

ADBLOGFILE=/Library/Caches/com.purplecomputing.mdm/Logs/AdobeDLProgress.log
...
customadobecc)
	...
	#appName="Adobe Package Downloader.app"
	type="pkgInZip"
	if [[ $(arch) == "arm64" ]]; then
		downloadURL="https://$CUSTOMSRVUSER:$CUSTOMSRVPASS@$CUSTOMSRVHOST/APPS/ADOBE/ARM/INSTALL.pkg.zip"
	elif [[ $(arch) == "i386" ]]; then
...
echo "*** END adobe-custom-install.sh ***"
